...
public class ActionViewPreparator {
  ...
  private <E, D extends BaseActionDto<E, D>, T> void setCurrentTags(
    ...
    List<Integer> tagIdList = junctionService.prepareTagIdLitByActionId(currentDto.getId());
    // tagをリストで持つように変更予定。
  ...
  public void prepareCurrentDoneView(
    ...
    for(int doneId : doneIds){
      List<Integer> tagIdList = doneTagJunctionService.prepareTagIdLitByActionId(doneId);
      /*
       tagをリストで持つように変更予定。
